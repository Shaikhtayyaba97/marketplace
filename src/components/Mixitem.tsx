...
const Mixitem = ({ category }: { category: string }) => {
  ...
  return (
    <div className="container mx-auto p-6 bg-white text-black">
      <div className="mb-6">
        <h1 className="text-3xl font-bold text-left mb-2">Most wanted</h1>
        <p className="text-left text-gray-600">
        Discover our most wanted collection featuring trendy rings, elegant bangles, and stylish bracelets for both men and women. Perfect for any occasion, these timeless pieces add sophistication to your look.
        </p>
      </div>
        ...
        {sortedProducts.map((product) => (
          <li key={product._id} className="bg-white shadow-lg rounded-md p-4 transition-all duration-300 hover:shadow-xl">
            ...
            <div className="relative overflow-hidden group">
              <Link href={`/men/mix/${product.slug.current}`}>
                <Image
                  src={product.image}
                  alt={product.name}
                  width={250}
