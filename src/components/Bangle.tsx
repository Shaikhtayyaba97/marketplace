...
const Bangle = async ({ category }: { category: string }) => {
  ...
  return (
    <div className="container mx-auto p-6 bg-white text-black">
      <h1 className="text-3xl font-bold text-left mb-2">Bangles</h1>
      <p className="text-left text-gray-600">
      Waterproof, Stainless Steel bangle, Tarnish Free and Color Guaranteed for Long-Lasting Wear.
      </p>
      ...
      <ul className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {products.map((product) => (
          <li key={product._id} className="bg-white shadow-lg rounded-md p-4 hover:shadow-xl transition-all duration-300">
            <Link href={`/women/bangle/${product.slug.current}`}>
              <Image
                src={product.image}
                alt={product.name}
                width={250}
