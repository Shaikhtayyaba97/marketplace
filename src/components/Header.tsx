...
const Header = () => {
  ...
  return (
    <header className="bg-white text-black px-6 py-4 flex justify-between items-center">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="absolute top-16 left-0 w-full bg-white text-black p-6 space-y-4 md:hidden z-10">
          <Link href="/women" className="block hover:text-gray-300">Women</Link>
          <Link href="/men" className="block hover:text-gray-300">Men</Link>
          <Link href="/kids" className="block hover:text-gray-300">Kids</Link>
          <Link href="/customize" className="block hover:text-gray-300">Customize</Link>
        </div>
      )}
    </header>
  );
